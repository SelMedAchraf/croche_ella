...
const TabLoading = () => (
  <div className="py-20 flex flex-col items-center justify-center">
    ...
    <p className="mt-4 text-text/60 animate-pulse">Loading management tools...</p>
  </div>
);
      ...
      <div className="flex-1 ml-20 transition-all duration-300 min-w-0 flex flex-col min-h-screen">
        ...
        <div className="flex-1 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <StatCard icon={<FiPackage className="w-8 h-8" />} title="Total Products" value={stats.totalProducts} color="bg-blue-500" />
            <StatCard icon={<FiBox className="w-8 h-8" />} title="Total Items" value={stats.totalItems} color="bg-purple-500" />
            <StatCard icon={<FiDroplet className="w-8 h-8" />} title="Total Colors" value={stats.totalColors} color="bg-pink-500" />
            <StatCard icon={<FiClock className="w-8 h-8" />} title="Pending Orders" value={stats.pendingOrders} color="bg-yellow-500" />
          </div>
          ...
          <div className="bg-white rounded-lg shadow-sm border mb-6">
            <div className="p-4 sm:p-6 min-h-[400px]">
              <Suspense fallback={<TabLoading />}>
                {activeTab === 'products' && (
                  <ProductsTab products={products} onRefresh={fetchData} setZoomedImage={setZoomedImage} />
                )}
                {activeTab === 'orders' && (
                  <OrdersTab orders={orders} onRefresh={fetchData} />
                )}
                {activeTab === 'categories' && (
                  <CategoriesTab onRefresh={fetchData} />
                )}
                {activeTab === 'items' && (
                  <ItemsTab setZoomedImage={setZoomedImage} />
                )}
                {activeTab === 'deliveryPrices' && (
                  <DeliveryPricesTab />
                )}
                {activeTab === 'colors' && (
                  <ColorsTab />
      ...
      >
        ...
        >
          <img
            src={imageUrl}
            alt="Zoomed"
            className="rounded-lg shadow-2xl max-h-[85vh] w-auto object-contain bg-white"
          />
          <button
            onClick={onClose}
            className="absolute -top-4 -right-4 md:top-2 md:right-2 p-3 bg-white/10 hover:bg-white/20 text-white rounded-full transition-colors backdrop-blur-md border border-white/20 shadow-lg group"
          >
